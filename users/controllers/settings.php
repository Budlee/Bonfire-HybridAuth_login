...
//end user_status()
    ...
    public function hybridauth_managment()
    {
        $this->load->library('HybridAuthLib');

        if ($this->input->post('enable_save'))
        {
            $this->settings_lib->set('hybridauth_enabled', $this->input->post('enable'), 'users');
        }
        if ($this->input->post('save'))
        {
            $this->save_hybrid_setup();
        }

        Template::set('hybridauth_enabled', $this->settings_lib->item('hybridauth_enabled') == 1 ? TRUE : FALSE);
        Template::set('providers', $this->user_model->get_providers());
        Template::set('toolbar_title', lang('us_hybrid_auth_management'));
        Template::render();
    }

    //--------------------------------------------------------------------

    private function save_hybrid_setup()
    {
        $providers = $this->user_model->get_providers();
        $data = array();
        foreach ($providers as $provider => $value)
        {
            if ($this->input->post($provider . '_enabled') !== FALSE)
            {
                if ($value['enabled'] == 0)
                {
                    $data[$provider]['enabled'] = 1;
                }
            }
            else
            {
            ...
            if ($this->input->post($provider . '_key') !== FALSE)
            {
                if ($value['keys']['key'] != $this->input->post($provider . '_key'))
                {
                    $data[$provider]['key'] = $this->input->post($provider . '_key');
                }
            }
            if ($this->input->post($provider . '_secret') !== FALSE)
            {
                if ($value['keys']['secret'] != $this->input->post($provider . '_secret'))
                {
                    $data[$provider]['secret'] = $this->input->post($provider . '_secret');
                }
            }
        }
        if (count($data) == 0)
        {
            Template::set_message(lang('us_ha_no_update'), 'info');
        }
        else
        {
            $this->user_model->update_hybridauth($data) === TRUE ? Template::set_message(lang('us_ha_update_success'), 'success') : Template::set_message(lang('us_ha_update_error'), 'error');
        }
    }
