...
class Users extends Front_Controller
{
    ...
    public function __construct()
    {
        ...
        Assets::add_module_js('users', 'facebook.js');
    }
...
//end __construct()
    ...
    public function login()
    {
        ...
        if ($this->auth->is_logged_in() === FALSE)
        {
            ...
            //Hybrid_auth
            if ($this->settings_lib->item('hybridauth_enabled') == 1)
            {
                Template::set('hybrid_auth_providers', $this->user_model->get_providers());
            }
    ...
    public function profile()
    {
        ...
        if (isset($_POST['save']))
        {
            ...
            if ($this->save_user($user_id, $meta_fields))
            {
                ...
                {
                    if ((!isset($field['admin_only']) || $field['admin_only'] === FALSE || (isset($field['admin_only']) && $field['admin_only'] === TRUE && isset($this->current_user) && $this->current_user->role_id == 1)) && (!isset($field['frontend']) || $field['frontend'] === TRUE) && $this->input->post($field['name']) !== FALSE)
                    {
...
//end save_user()
    ...
    public function hybrid_login($provider)
    {
        try
        {
            ...
            {
                log_message('error', lang('us_ha_disabled_provider') . $provider);
                Template::set_message(lang('us_ha_provider_unavaliable'));
                Template::redirect(REGISTER_URL);
            }
        }
        catch (Exception $e)
        {
            ...
            switch ($e->getCode())
            {
                case 0 : $error = lang('us_ha_error_unknown');
                    break;
                case 1 : $error = lang('us_ha_error_configuration');
                    break;
                case 2 : $error = lang('us_ha_error_provider_config');
                    break;
                case 3 : $error = lang('us_ha_error_provider_unkown');
                    break;
                case 4 : $error = lang('us_ha_error_provider_missing_credentials');
                    break;
                case 5 : log_message('debug', lang('us_ha_error_connection_cancelled'));
                    if (isset($service))
                    {
                        log_message('debug', lang('us_ha_logout'));
                        $service->logout();
                    }
                    Template::set_message(lang('us_ha_cancelled') . $provider);
                    Template::redirect(REGISTER_URL);
                    break;
                case 6 : $error = lang('us_ha_error_user_profile_fail');
                    break;
                case 7 : $error = lang('us_ha_error_user_not_connected');
                    break;
            }
            ...
            Template::set_message(lang('us_ha_authenticate_error'));
            Template::redirect(REGISTER_URL);
        }
    }

    private function register_provider_user($provider, $user)
    {
        $this->load->helper('email');
        //Checks for username and email to get the user into our system. The key is the email
        //as the password and username can be made up as the login auth will not attempt any login
        //if they user a provider
        if (!isset($user->email) || !valid_email($user->email))
        {
            Template::set_message(lang('us_ha_invalid_email'), 'error');
            Template::redirect(REGISTER_URL);
        }
        if (!$this->user_model->is_unique('email', $user->email))
        {
            Template::set_message(lang('us_ha_email_not_unique'), 'error');
            Template::redirect(LOGIN_URL);
        }
        ...
        if (empty($username))
        {
            $username = random_string('alpha', 8);
        }
        if (!$this->user_model->is_unique('username', $username))
        {
            ...
            if ($index >= 10)
            {
                Template::set_message(lang('us_ha_username_fail'), 'error');
                return;
            }
        }
    ...
    public function endpoint()
    {
        ...
        require_once APPPATH . 'modules/users/third_party/hybridauth/index.php';
    }
