...
//end admin_deactivation()
    ...
    protected $table_hybridauth = 'hybrid_auth_settings';


    public function get_config()
    {
        $query = $this->db->get($this->table_hybridauth);
        $return_array['providers'] = array();
        foreach ($query->result() as $provider)
        {
            $return_array['providers'][$provider->provider] = array(
                    'enabled' => ($provider->enabled == 1 ? TRUE : FALSE )
            );
            if($provider->key !== "NULL")
            {
                $return_array['providers'][$provider->provider]['keys'] = array('id' => $provider->key, 'key' => $provider->key, 'secret' => $provider->secret);
            }
            if($provider->scope !== "NULL")
            {
                $return_array['providers'][$provider->provider]['scope'] = $provider->scope;
            }
        }
        return $return_array;
    }
    ...
    public function get_providers()
    {
        ...
        foreach ($query->result() as $provider)
        {
            $return_array[$provider->provider] = array('enabled' => ($provider->enabled == 1 ? TRUE : FALSE ));
            if($provider->key !== "NULL")
            {
                $return_array[$provider->provider]['keys'] = array('id' => $provider->key, 'key' => $provider->key, 'secret' => $provider->secret);
            }
            if($provider->scope !== "NULL")
            {
                $return_array[$provider->provider]['scope'] = $provider->scope;
            }
        }
        return $return_array;
    }
    
    public function update_hybridauth($data)
    {
        $s = TRUE;
        foreach ($data as $provider => $values)
        {
            $this->db->where(array('provider'=>$provider));
            $this->db->update($this->table_hybridauth, $values);
            if($s === TRUE)
            {
                if($this->db->affected_rows() <= 0)
                {
