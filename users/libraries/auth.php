...
//end __construct()
     ...
     * @return bool
     */
    public function login($login, $password, $remember = FALSE, $hybridauth_login = FALSE)
    {
        $user = FALSE;
        if (empty($login) || empty($password))
        {
            $user = $this->get_hauth_user();
            if ($user === FALSE && $hybridauth_login == FALSE)
            {
            ...
            else if ($user === FALSE && $hybridauth_login == TRUE)
            {
                Template::set_message(lang('us_ha_login_fail'), 'error');
                return FALSE;
            }
            //Hybrid_auth login. Condensed version of below for hauth
        ...
        //Check to see if its a HybridAuth login account
...
//end user_id()
     ...
     * @access public
     *
     * @return string The identity used to login OR array containing the provider.
     */
    public function identity()
    {
        ...
        if ($user !== FALSE)
        {
            return array('provider' => $user->hauth_provider);
        }
    ...
    private function autologin()
    {
        ...
        if ($query->num_rows() == 1)
        {
            ...
            if (!$this->ci->session->userdata('user_id'))
            {
                ...
                if (!$user)
                {
                    //Check for Hauth login
                    $user = $this->get_hauth_user();
                    if ($user !== FALSE)
                    {
                        $this->setup_session($user->id, $user->username, $user->password_hash, $user->email, $user->role_id, TRUE, '', $user->username);
                    }
                    return;
                }
...
//end _identity_login()
    ...
    /**
     * Checks HybridAuth to see if user is logged into account.
     *
     * @access private
     *
     * @return object (or a false value)
    ...
    private function get_hauth_user()
    {
        ...
        {
            $this->ci->benchmark->mark('hybridauth_auth_start');
            $pro_storage = $this->ci->hybridauthlib->authenticate($providers[0]);
            $this->ci->benchmark->mark('hybridauth_auth_end');
            if ($pro_storage->isUserConnected())
            {
                $this->ci->benchmark->mark('hybridauth_profile_start');
                $user = $this->ci->user_model->find_by('email', $pro_storage->getUserProfile()->email);
